...
public class ServicePersistenceAdapter implements ServicePersistencePort {
    ...
    private final CategorySelectorAdapter categoryRepository;
    private final ServiceMapper mapper;

    public ServicePersistenceAdapter(ServiceMySQLRepository serviceRepository, CategorySelectorAdapter categoryRepository, ServiceMapper mapper) {
    ...
    public ServiceData newService(NewServiceDTO serviceDTO) {
        CategoryEntity category = this.categoryRepository.getCategoryEntityById(serviceDTO.getCategoryId());
    ...
    public ServiceData update(ServiceData serviceBase) {
        CategoryEntity category = this.categoryRepository.getCategoryEntityById(serviceBase.getCategory().getId());
